...
function TermCell({
  ...
  highlightedCourses, isDragging, onDragStart, onDragEnd,
}: TermCellProps) {
  const [isOver, setIsOver] = useState(false);
  const units = termUnits(courseIds);
...
export default function ScheduleGrid({
  ...
  highlightedCourses, dragPayload, onDragStart, onDragEnd,
}: Props) {
  const totalUnits = Object.values(schedule).reduce(
    (sum, terms) => sum + Object.values(terms).reduce((s, ids) => s + termUnits(ids), 0),
    0
  );
